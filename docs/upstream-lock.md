...
| lotem/librime-octagram | `bfb168ca33d8b372596fdf2007933f3da1cf360e` | GPL-3.0 | External AutoDL build for the Wanxiang grammar baseline |
...
The rime-ice baseline is deliberately reduced to the core `script_translator` in `fixtures/rime/tinyrime_ice.schema.yaml`: Lua, emoji, private custom phrases, and user learning are excluded for deterministic headless fixtures. The strong baseline adds locked librime-octagram and the 420,250,668-byte `wanxiang-lts-zh-hans.gram` (SHA-256 `01ffe37f22607bf8a5cd5d82a3349f6df97744369464aee4577585112d85469d`) through `fixtures/rime/tinyrime_ice_octagram.schema.yaml`. Neither large model is vendored in Git.
