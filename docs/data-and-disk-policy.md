...
The active research source is the user-selected `fjcanyue/wikipedia-zh-cn` dataset snapshot `wikipedia-zh-cn-20260501.json`, downloaded only to AutoDL through the temporary `HF_ENDPOINT=https://hf-mirror.com`. It is JSON Lines with document `id`, `title`, `tags`, and `text`. The full source remains remote. A fixed-seed, one-pass document reservoir samples across the entire file before the existing document-ID split and candidate-window generation; candidate generation stops at 100k/10k/10k and never precomputes all Wikipedia candidates.
